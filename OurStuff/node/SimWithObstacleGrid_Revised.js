...
function initialize(){
	let custommethods = {
			...
			drawBelow : drawBelow,
			postMCSListener: trackVelocities
		 }
	...
	// step()
	sim.previousPositions = {};
	sim.run()
}

function trackVelocities() {
    let cellpixels = this.C.getStat(CPM.CentroidsWithTorusCorrection);
    let velocities = {};
    
    for (let cid in cellpixels) {
        if (this.previousPositions[cid]) {
            let prev = this.previousPositions[cid];
            let curr = cellpixels[cid];
            velocities[cid] = [curr[0] - prev[0], curr[1] - prev[1]];
        }
        this.previousPositions[cid] = cellpixels[cid];
    }
    
    this.velocities = velocities;
}

// Cim is the canvas for simulation

// TODO: Fix this broken function that is supposed to plot the directions of the cells
function drawBelow() {
    this.Cim.drawPixelSet(this.channelvoxels, "0000FF");
    
    if (this.velocities) {
        for (let cid in this.velocities) {
            let velocity = this.velocities[cid];
            let centroid = this.C.getStat(CPM.CentroidsWithTorusCorrection)[cid];
            let end = [centroid[0] + velocity[0] * 5, centroid[1] + velocity[1] * 5];
			
			this.Cim.beginPath();
            this.Cim.moveTo(centroid[0], centroid[1]);
            this.Cim.lineTo(end[0], end[1]);
            this.Cim.strokeStyle = "#FF0000"; // Red color
            this.Cim.lineWidth = 2;
            this.Cim.stroke()

            this.Cim.drawLine(centroid, end, "FF0000");
        }
    }
}
